...
| Destructive actions are explicit and guarded. | `src/features/vault-management/ui/actions.js` uses confirmations, undoable vault deletion where possible, and cross-tab vault-change notifications with a Chrome local-storage fallback; tests cover empty/canceled destructive states and cross-tab invalidation. |
| Long URLs, labels, and bulk controls stay inside the viewport. | `src/styles/tokens.css` prevents page-level horizontal overflow; `src/styles/results.css` wraps result titles, URLs, metadata, and rule values; `src/styles/responsive.css` keeps History bulk actions readable at narrow widths; `npm run validate` enforces these guardrails. |
