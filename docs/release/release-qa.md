...
- Manual release check: pause the local focus blockers, open `chrome://extensions`, enable Developer mode, load this repository folder unpacked, then verify the toolbar action opens BrowseVault, History bulk-action buttons stay compact at 480 px and 640 px, a second non-BrowseVault active tab creates another BrowseVault tab, deleting a vault record in one BrowseVault tab refreshes the other BrowseVault tab, and Chrome Web Store screenshots still match the current UI and store listing copy.
