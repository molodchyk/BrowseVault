...
- `src/features/backup-import/` owns archive import/export actions, export download helpers, export filename rules, file parsing, import normalization, import planning, import summary logic, integrity metadata, import-preview display state, and restore-flow rendering.
...
- `scripts/check-locales.mjs` verifies manifest `__MSG_*__` references, static extension-page localization bindings, dynamic Quick Open keys, dynamic display-preference status and backup-summary keys, and dynamic history-result UI status keys against `_locales/en/messages.json`, then rejects unresolved or unused locale keys.
