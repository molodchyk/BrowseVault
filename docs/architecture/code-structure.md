...
- `src/storage.js` owns IndexedDB transactions, metadata persistence, rule persistence, vault mutations, and compatibility exports for storage-facing callers.
...
- `src/features/vault-management/` owns extension-page vault deletion, Chrome-history deletion requests, undo, reset, domain/category-rule actions, rule normalization, vault record normalization, retention cleanup, and duplicate cleanup.
...
- Continue splitting `src/storage.js` into feature storage models and a thinner IndexedDB platform/storage layer; it is now below the hard limit, but still above the 600-line split-follow-up threshold.
