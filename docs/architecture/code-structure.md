...
- `scripts/check-locales.mjs` verifies manifest `__MSG_*__` references, static extension-page localization bindings, dynamic Quick Open keys, dynamic display-preference status keys, and dynamic history-result UI status keys against `_locales/en/messages.json`, then rejects unresolved or unused locale keys.
